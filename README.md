...
7. 真正写入前会重新读取集群。审批期间如果发布了新版本、修改了副本数、删除了健康证明或读取失败，旧审批立即失效，不执行写操作。
...
8. **旧审批不能覆盖新发布**：批准后会重新查询 rollout history。集群语义状态有变化就停止；通过后写操作仍携带最新 resourceVersion，防止校验与写入之间的并发覆盖。
