...
The focused console receives a global server-sent incident feed, so a newly accepted alert is
selected without waiting for polling. The preflight trace correlation, graph nodes, and bounded
tool calls are shown as they start and finish, including nested Kubernetes, Prometheus, Loki,
Tempo, and Git reads, duration, quality-gate branches, approval pauses, mutating actions, and
verification. UI copy stays provider-neutral (`Agent 正在分析`) so changing the configured model
does not change the product narrative.
