    ...
    async def _diagnose(self, state: IncidentState) -> dict[str, Any]:
        ...
        try:
            diagnosis = await self.provider.structured(
                ...
                "只有原始观测中出现与当前假设直接相关的明确失败信号时，Evidence 的 "
                "supports_hypothesis 才能为 true。Ready Pod、空 Event、空日志、stable/healthy "
                "rollout、空指标或正常值都不能作为故障支持证据；它们只能作为反证并设置 "
                "supports_hypothesis=false，或者从该假设中省略。不得为了满足多源要求而把"
                "健康、空白或仅表示资源存在的观测写成支持证据。"
